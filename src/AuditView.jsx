...
const AuditView = () => {
    ...
    useEffect(() => {
        const fetchAudit = async () => {
            if (!apiUrl) return;
            try {
                const res = await fetch(`${apiUrl}?action=GET_AUDIT`);
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-slate-700 bg-slate-900/50 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em]">Registro de Auditoría de Datos</p>
                    </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-900/80 text-[10px] text-slate-500 uppercase font-black tracking-widest border-b border-white/5">
                            <tr>
                                ...
                                <th className="px-6 py-5">Acción Ejecutada</th>
